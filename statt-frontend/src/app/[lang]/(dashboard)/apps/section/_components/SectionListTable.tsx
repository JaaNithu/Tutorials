...
// Type Imports
import type { SectionType } from '@/types/apps/userTypes'
import type { Locale } from '@configs/i18n'

// Component Imports
import AddNewUser from './AddNewSection'
import { getLocalizedUrl } from '@/utils/i18n'

// Style Imports
import tableStyles from '@core/styles/table.module.css'
import React from 'react'
import ViewUser from './ViewSection'
import EditUser from './EditSection'
import ConfirmationModal from './DeleteSection'
...
type UsersTypeWithAction = SectionType & {
  action?: string
}
...
const UsersList = () => {
  ...
  const handleDeleteSection = async (id: string) => {
    try {
        await axios.delete(`http://localhost:3001/section/delete/${id}`);
        setUsers((prevSections) => prevSections.filter((section) => section.id !== Number(id)));
  ...
  const columns = useMemo<ColumnDef<UsersTypeWithAction, any>[]>(() => [
    ...
    columnHelper.accessor('isActive', {
      ...
      cell: info => (
        <Typography
          sx={{
            backgroundColor: info.getValue() ? '#388E3C' : '#E53935',
            color: 'white',
            padding: '4px 8px',
            borderRadius: '20px',
            textAlign: 'center',
      ...
      cell: ({ row }) => (
        <div className='flex items-center gap-0.5'>
          ...
          <IconButton
            size='small'
            onClick={() => {
              console.log('View icon clicked for ID:', row.original.id);
  ...
  ], [handleEditUser, setSelectedUserId, setViewUserOpen]);
  ...
  if (loading) {
    return <Typography>Loading section...</Typography>
  }
  ...
  return (
    <>
      <Card>
        <CardHeader title='Section Data Table' className='pbe-4' />
        <Divider></Divider>
      ...
      <EditUser
        ...
        sectionId={selectedUserId}
      />

        <ViewSection 
          open={viewUser} 
          handleClose={() => setViewUserOpen(false)} 
          sectionId={selectedUserId} 
        />
